<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-row w-full justify-between items-center">
            ...
            <a href="{{ route('admin.products.index') }}" class="font-bold py-3 px-6 rounded-full text-white bg-red-700">Back</a>
        </div>
    </x-slot>
                ...
                <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data">
                    ...
                    <div class="mt-4">
                        ...
                        <select name="category_id" id="category_id" class="py-3 rounded-lg pl-3 w-full border border-slate-300">
                            <option value="{{ $product->category->id }}" @if(old('category_id') == '') disabled selected @endif>
                    ...
                    <div class="mt-4">
                        ...
                        <textarea id="about" name="about" cols="30" rows="5" class="border border-slate-300 rounded-xl w-full">{{ $product->about }}</textarea>
                        <x-input-error :messages="$errors->get('about')" class="mt-2" />
                    </div>

                    <div class="mt-4">
                        <x-input-label for="photo" :value="__('Photo')" />
                        <img src="{{ Storage::url($product->photo) }}" alt="" class="w-[50px] h-[50px] mt-2 mb-3">
